...
export const getInitialDriversForUser = (email?: string, name?: string): Driver[] => {
  ...
  if (name && name.trim() !== '') {
    ...
    if (cleaned.toLowerCase() === 'hugovieira') {
      cleaned = 'Hugo';
    }

    primaryName = cleaned.length > 0 ? cleaned : 'Hugo';
  }
